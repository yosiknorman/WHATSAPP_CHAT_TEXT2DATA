...
yup = list()
system(paste0("mkdir digitize/se_Kab/", pilih_tahun,pilih_bulan))
for(i in 1:length(dayys)){
  ...
  write.table(file = paste0("digitize/se_Kab/", pilih_tahun,pilih_bulan, "/warning_whatsapp_", pilih_tahun, pilih_bulan, dayys[i], ".txt") , yup[[i]], col.names = T, row.names = F)
}
